...
module.exports = function gen(config, func) {
    ...
    const returnType = formatter.csType(func.returnDataType, true);
    ...
    code.appendLine(1, "{");
    code.appendLine(2, `${returnType} ${func.name}(${csArgString});`);
    code.appendLine(1, "}");
    code.appendLine();
    code.appendLine(1, `public partial class ${config.efDbContextName}`);
    code.appendLine(1, "{");
    code.appendLine(2, `public ${returnType} ${func.name}(${csArgString})`);
    code.appendLine(2, "{");
    ...
    if (parms.length) {
        ...
        code.appendLine();
        code.appendLine(3, `return Database.SqlQuery<${returnType}>("select dbo.${func.name}(${sqlParmString})", new object[]`);
        code.appendLine(3, "{");
        code.appendLine(4, `${csParmString}`);
        code.appendLine(3, "}).Single();");
    } else {
        code.appendLine(3, `return Database.SqlQuery<${returnType}>("select dbo.${func.name}()").Single();`);
    }
